...
from KCA_dataloader import get_data_loader
import KCA_evaluation as evaluation
import warnings
from config import *
...
from KCA_OutRawTransformerModel import BCQ_Attention_LSTM
import train_utils
...
def main():
    print("------------------------------------------------------------------------------")
    print("-----KCA attention after lstm-----------------------------")
    print("------------------------------------------------------------------------------")
    print(__name__)
    model_type = "KCA"

    model_type_subfix = f"[{KCA_EPOCH}_{BCQL_LAYER}]"
    ...
    for fold in range(10):
        ...
        #                             config.questions,
        #                             node_count, path_count, device)
        model = BCQ_Attention_LSTM(BCQ_config.before_trans_alignment_d,
                                    config.questions * 2,
                                    config.hidden,
                                    config.layers,
                                    config.questions,
        ...
        optimizer = optim.Adam(model.parameters(), lr=KCA_config.lr)
        loss_func = evaluation.lossFunc(config.questions, config.length, device)
        all_epoch_loss = []
        for epoch in range(KCA_EPOCH):
